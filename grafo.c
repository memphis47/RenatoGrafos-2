	...
	for (int i = 0; i < g->n_vertices; ++i)
	{
		...
		for (int j = 0; j < g->n_vertices; ++j)
		{
			if(g->pathSize[i][j]>=1){
				...
				int tam= (int) strlen(newGrapf->vertices[j]->nome)+1;
				newGrapf->vertices[i]->arestas[index].verticeLigado=(char *) malloc(sizeof(char)*tam);
        if(newGrapf->vertices[i]->arestas[index].verticeLigado!=NULL)
				  newGrapf->vertices[i]->arestas[index].verticeLigado=newGrapf->vertices[j]->nome;
